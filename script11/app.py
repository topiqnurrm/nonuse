...
def get_mahasiswa():
    try:
        conn = create_connection()
        if conn:
            cursor = conn.cursor(dictionary=True)  # Gunakan dictionary=True agar hasilnya berbentuk dictionary
...
def vikor(mahasiswa):
    ...
    kriteria = ['nilai_ipk', 'nilai_organisasi', 'nilai_keaktifan', 'nilai_kepemimpinan']

    # Menghitung nilai terbaik dan terburuk
    best = {k: max([m[k] for m in mahasiswa if m.get(k) is not None]) for k in kriteria}
    worst = {k: min([m[k] for m in mahasiswa if m.get(k) is not None]) for k in kriteria}
    
    s = []
    r = []
    
    for m in mahasiswa:
        S_i = sum([(best[k] - m[k]) / (best[k] - worst[k]) if (best[k] - worst[k]) != 0 else 0 for k in kriteria])
        R_i = max([(best[k] - m[k]) / (best[k] - worst[k]) if (best[k] - worst[k]) != 0 else 0 for k in kriteria])
        s.append(S_i)
        r.append(R_i)
    ...
    Q = [
        (0.5 * (S_i - S_best) / (S_worst - S_best) if S_worst - S_best != 0 else 0) + 
        (0.5 * (R_i - R_best) / (R_worst - R_best) if R_worst - R_best != 0 else 0) 
    ...
    mahasiswa.sort(key=lambda x: x['Q'])  # Sort by Q value
...
def result():
    mahasiswa = get_mahasiswa()
    hasil_vikor = vikor(mahasiswa)  # This will sort by Q
    return render_template('2_result.html', mahasiswa=hasil_vikor)


# route untuk halaman data mahasiswa
@app.route('/data_mahasiswa')
def data_mahasiswa():
    mahasiswa = get_mahasiswa()  # Retrieve mahasiswa data without sorting
    return render_template('2_index.html', mahasiswa=mahasiswa)
